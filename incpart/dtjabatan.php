...
<div class="row">
	<div class="col-md-12">
		<button href="#" class="btn btn-outline-success btn-lg" data-toggle="modal" data-target="#exampleModalLong" role="button" data-backdrop="static" data-keyboard="false"><i class="fa fa-fw fa-user-plus"></i> Tambah Jabatan</button>
	</div>
</div>
...
<tbody>
<?php $a = mysql_query("select * from dt_jabatan");
while($b = mysql_fetch_array($a))	{ ?>
  <tr>
	<td><?= $b[1]; ?></td>
	<td>
		<button class="btn btn-info btn-sm" data-toggle="modal" data-target="#exampleEdit" data-id="<?= $b[0]; ?>" data-nya="<?= $b[1]; ?>" data-nya2="<?= $b[2]; ?>" title="Perbaharui Data" data-backdrop="static" data-keyboard="false">
      ...
      <div class="modal-body">
        ...
        <!--<form> -->
          <div class="form-group">
            <label for="jabatan" class="form-control-label">Nama Jabatan</label>
            <input type="text" class="form-control" name="jabatan" id="jabatan">
          </div>
          <div class="form-group">
          	  <label for="ha" class="form-control-label">Hak Akses Lihat Kerja</label>
			  <div class="form-check">
				<label class="custom-control custom-radio">
				  <input id="ha" name="ha" type="radio" class="custom-control-input" value="0" checked>
				  <span class="custom-control-indicator"></span>
				  <span class="custom-control-description">Biasa</span>
				</label>
				<label class="custom-control custom-radio">
				  <input id="ha" name="ha" type="radio" class="custom-control-input" value="1">
				  <span class="custom-control-indicator"></span>
				  <span class="custom-control-description">Seluruhnya (Kadis)</span>
				</label>
			  </div>
		  </div>
        <!--</form> -->
...
<script>
	$(document).ready(function(){
		$("#btnDo").click(function(){
        	...
        	data.append('var1', $("#jabatan").val());
			data.append('var2', $("input[name=ha]:checked").val());
			
			var txt = '<i class="fa fa-plus" aria-hidden="true"></i> Tambah Data';
			$.ajax({
				url: 'incdo/dojabatan.php?do=add', // File tujuan
				...
				success: function(response){
					if(response.status=='ok'){
						...
						setTimeout(function() {
							$('#ui-view').load("incpart/dtjabatan.php");
						}, 500);
					}
					else{
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <!--<form>-->
          <div class="form-group">
            <label for="jabatans" class="form-control-label">Nama Jabatan</label>
            <input type="text" class="form-control" name="jabatans" id="jabatans">
          ...
          <div class="form-group">
			  ...
			  <div class="form-check">
				<label class="custom-control custom-radio">
				  <input id="haa" name="haa" type="radio" class="custom-control-input" value="0">
				  <span class="custom-control-indicator"></span>
				  <span class="custom-control-description">Biasa</span>
				</label>
				<label class="custom-control custom-radio">
				  <input id="haa" name="haa" type="radio" class="custom-control-input" value="1">
				  <span class="custom-control-indicator"></span>
				  <span class="custom-control-description">Seluruhnya (Kadis)</span>
				</label>
			  </div>
...
<script>
	...
	$(document).ready(function(){
		$("#btnEdt").click(function(){
			...
			var data = new FormData();
        	data.append('var1', $("#jabatans").val());
			data.append('var2', $("input[name=haa]:checked").val());
			data.append('id', $("#id").val());
			
			var txt = '<i class="fa fa-pencil-square-o" aria-hidden="true"></i>  Perbaharui Data';
			$.ajax({
				url: 'incdo/dojabatan.php?do=update', // File tujuan
				type: 'POST', // Tentukan type nya POST atau GET
				data: data, // Set data yang akan dikirim
				processData: false,
				contentType: false,
...
<script>
	...
	$(document).ready(function(){
		$("#btnHps").click(function(){
			...
			$.ajax({
				url: 'incdo/dojabatan.php?do=delete', // File tujuan
				type: 'POST', // Tentukan type nya POST atau GET
				data: data, // Set data yang akan dikirim
				processData: false,
				contentType: false,
